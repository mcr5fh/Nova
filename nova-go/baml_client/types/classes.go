...
func (c *EscalationDecision) Decode(holder *cffi.CFFIValueClass, typeMap baml.TypeMap) {
	...
	for _, field := range holder.Fields {
		...
		case "action":
			c.Action = baml.Decode(valueHolder).Interface().(EscalationAction)
...
func (c *PlannerDecision) Decode(holder *cffi.CFFIValueClass, typeMap baml.TypeMap) {
	...
	for _, field := range holder.Fields {
		...
		case "size":
			c.Size = baml.Decode(valueHolder).Interface().(TaskSize)

		case "should_split":
			c.Should_split = baml.Decode(valueHolder).Bool()
...
type SubtaskDefinition struct {
	...
	Type        Union3KbugOrKfeatureOrKtask `json:"type"`
...
func (c *SubtaskDefinition) Decode(holder *cffi.CFFIValueClass, typeMap baml.TypeMap) {
	...
	for _, field := range holder.Fields {
		...
		case "type":
			c.Type = baml.Decode(valueHolder).Interface().(Union3KbugOrKfeatureOrKtask)
...
func (c *TelemetryEntry) Decode(holder *cffi.CFFIValueClass, typeMap baml.TypeMap) {
	...
	for _, field := range holder.Fields {
		...
		case "attempt_num":
			c.Attempt_num = baml.Decode(valueHolder).Int()

		case "model":
			c.Model = baml.Decode(valueHolder).Interface().(string)

		case "tokens_used":
			c.Tokens_used = baml.Decode(valueHolder).Int()

		case "duration_ms":
			c.Duration_ms = baml.Decode(valueHolder).Int()

		case "cost_usd":
			c.Cost_usd = baml.Decode(valueHolder).Float()

		case "success":
			c.Success = baml.Decode(valueHolder).Bool()
...
func (c *ValidationResult) Decode(holder *cffi.CFFIValueClass, typeMap baml.TypeMap) {
	...
	for _, field := range holder.Fields {
		...
		case "passed":
			c.Passed = baml.Decode(valueHolder).Bool()
